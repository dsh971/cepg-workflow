#!/usr/bin/env bash
# hook-extract.sh — SHARED JSON + host-detection helpers for cep PreToolUse hooks.
...
# gstack_* to cep_* (cosmetic rebrand only — detection/parsing logic is
# unchanged) and GSTACK_HOME becomes CEP_HOME.

# cep_hook_extract_field PAYLOAD FIELD
...
# cep_hook_decision DECISION REASON
...
cep_hook_decision() {
  _chd_decision="$1"
  _chd_reason="$2"
  _chd_encoded=$(cep_hook_json_string "$_chd_reason")
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"%s","permissionDecisionReason":%s}}\n' "$_chd_decision" "$_chd_encoded"
}

# cep_hook_decision_ask REASON
...
#   paths) routes through this helper rather than calling cep_hook_decision
...
#   cep_hook_decision directly.
cep_hook_decision_ask() {
  _chda_reason="$1"
  if cep_is_codex; then
    cep_hook_decision deny "$_chda_reason [escalated to deny on Codex: PreToolUse \"ask\" is silently ignored there (openai/codex#28437) — this command is blocked outright rather than passing through unenforced]"
  else
    cep_hook_decision ask "$_chda_reason"
  fi
}

# cep_hook_log_fire SKILL PATTERN
#   Append a hook_fire analytics record (pattern name only, never command
#   content). Respects CEP_HOME so tests never pollute the operator's real
#   analytics file. Best-effort: failures never affect the hook decision.
cep_hook_log_fire() {
  _chlf_dir="${CEP_HOME:-$HOME/.cep}/analytics"
  ...
  printf '{"event":"hook_fire","skill":%s,"pattern":%s,"ts":"%s","repo":%s}\n' \
    "$(cep_hook_json_string "$1")" \
    "$(cep_hook_json_string "$2")" \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    "$(cep_hook_json_string "$_chlf_repo")" >> "$_chlf_dir/skill-usage.jsonl" 2>/dev/null || true
}
