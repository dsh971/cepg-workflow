...
- **Working plugin name: `cepg`** (originally inferred from this workspace's own directory name, `cep-workflow`, before the plugin identifier itself was renamed to `cepg` separately from the on-disk directory). A placeholder — trivially renamed before publishing, not a load-bearing decision.
...
**Dependencies:** U1
**Files:** `.cursor-plugin/plugin.json`, `.devin-plugin/plugin.json`, `.grok-plugin/plugin.json`, `.kimi-plugin/plugin.json`, `plugin.json` (root-level, Antigravity's manifest) plus `.agy/plugin.json` and `.agy/skills` as symlinks to it and to `skills/`, `.opencode/plugins/cepg.js`, `.cline/INSTALL.md` and `.cline/scripts/install-skills.sh`
...
- Whether `cepg` is the final plugin name, or a placeholder to rename before publishing.
