...
**Reviewability, when the branch's own history is noisy.** Phase 1's 2-3 logical commits are usually reviewable as-is; when they aren't (a branch with a long, tangled history from before this skill ran), mined from pstack's `make-pr-easy-to-review`: before rewriting anything, capture the pre-rewrite tree (`git rev-parse origin/<branch>^{tree}`) so the rewrite can be verified content-identical afterward (`git diff origin/<branch> --stat` should be empty); group commits in dependency order -- schema or generated definitions, core logic, wiring/integration, surface behavior, tests; and never fold a real behavior change into a commit framed as "cleanup." In the PR description, add a TL;DR that matches the actual diff, separate core files from generated/mechanical ones, and call out risky behavior changes, migration order, and test coverage explicitly. If the PR is too large to make reviewable even with this, recommend splitting it rather than polishing around the problem.
...
2. **Triage in priority order:**
   - **Merge conflicts** (`mergeStateStatus == DIRTY`): rebase or merge the base branch, then resolve each conflicting file with minimal, correctness-first edits, mined from pstack's `fix-merge-conflicts` -- prefer preserving both sides when safe, otherwise keep whichever variant compiles and preserves public behavior; regenerate lockfiles with the package manager's own tools rather than hand-editing them; never leave conflict markers in a staged file; avoid a broad refactor riding along with the resolution. Run compile/lint/tests before staging and summarize the resolution choices made. Force-push only if the branch is exclusively yours and not shared -- otherwise stop and ask.
   - **Failing checks:** inspect `gh pr checks --json name,bucket,state,workflow,link`, pull logs for the failing job (`gh run view <run-id> --log-failed`) or follow the check's own link when it isn't a GitHub Actions job, and extract the first actionable error -- mined from pstack's `fix-ci`. Fix one actionable failure at a time, preferring the smallest safe change over a broader refactor, then push and recheck the PR's check set. This is Phase 1's fix-rerun loop, re-entered as needed -- it does not require a second manual invocation of this skill.
...
Store the fingerprint at `${CEPG_HOME:-$HOME/.cepg}/ship/<repo-slug>/deploy-confirmed` (`<repo-slug>` is the sanitized `origin` remote, owner_repo with `/` replaced by `_`; fall back to the sanitized absolute repo path when there's no remote). Three states:
