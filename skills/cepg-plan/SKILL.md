...
**Artifact root note.** Every `docs/...` path below assumes the default artifact root. A project with a configured `docs_root` relocates all of them the same way — see [`artifact-root.md`](../../references/artifact-root.md).
...
**Beyond Deep.** For a large migration spanning many call sites, an ambitious multi-part change, or work a human will review only after stepping away — where even a Deep plan's structure undersells the rigor actually needed — apply this rigor escalation, mined from pstack's `figure-it-out`, on top of the plan Phase 4 still produces: state the definition of done as a falsifiable predicate up front, not "done well"; sequence Implementation Units riskiest-unknown-first so option value stays high, rather than dependency-convenience-first; call for the verification harness itself to be built (and a pre-change baseline captured) before the feature-bearing units, so each unit's check reads as old-value-vs-new-value; and note in the plan that the implementer should keep a running decision log, mined from pstack's `show-me-your-work`: a single TSV, one row per decision (`ts`, `phase`, `decision`, `why`, `evidence`, `result`), evidence as a pointer -- a commit SHA, PR number, or `file:line` -- never a paragraph, written as it lands rather than reconstructed at the end. Append-only: a wrong call gets a new row that supersedes it, never an edit to history. Keep it local by default; commit it only when the work is ambitious enough that a reviewer needs the trail to trust the result, same as the rigor threshold above. This changes how Phase 4's units are sequenced and what Phase 4's Verification fields ask for — it doesn't replace Implementation Units with something else.
...
For an unfamiliar or genuinely complex subsystem this plan will touch, build a plain mental model before structuring units, rather than planning against a surface reading — mined from pstack's `teach`: get oriented by reading the code, then work out both how it actually works and why it's shaped that way, and combine the two into the smallest complete account, not a changelog of functions and constants. This is proportional, not a standing step — skip it for anything small or already well understood, and don't let it turn into a second research pass on top of the one above.
...
For a shape targeting a statically-typed language, name it against these rules, mined from pstack's `principle-type-system-discipline`, not just its fields: model variants as a sum type rather than a bag of optional fields that admits contradictory combinations (a `{ completed: boolean; completedAt?: Date }` pair over-admits `completed: true` with no date -- the shape should make that combination unwritable, not merely undesirable); brand semantic primitives that share an underlying type but aren't interchangeable (a `UserId` and an `OrderId` are both strings underneath); name where untyped external data (a payload, a config value, a CLI arg) gets parsed into the shape at the boundary rather than trusted as-is past it; call out that any match over the shape's variants must be exhaustive, so a future variant fails compilation somewhere until handled; and derive the shape from an authoritative schema (a proto, an OpenAPI spec, a migration) instead of hand-rolling a parallel definition when one already exists. Strengthen a shape only where a runtime assertion or null check would otherwise be needed -- precision that isn't paying for a removed check is just ceremony.
