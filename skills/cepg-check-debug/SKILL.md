...
3. **Attempt reproduction.** Run the test, trigger the error, follow the reported steps -- whatever matches the input. Browser bugs: prefer `agent-browser` if installed, otherwise whatever the platform exposes. If reproduction needs conditions the agent can't create alone (specific data states, user roles, external services), write out the exact manual setup steps and walk the user through them rather than guessing.
