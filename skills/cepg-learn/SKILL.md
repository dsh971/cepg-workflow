...
**Artifact root note.** Every `docs/...` path below assumes the default artifact root; `CONCEPTS.md` always stays at the repo root regardless. A project with a configured `docs_root` relocates the rest the same way — see [`artifact-root.md`](../../references/artifact-root.md).
...
2. **Before drafting a prose edit, ask whether the fix can be structural instead** -- mined from pstack's `principle-encode-lessons-in-structure`: a lint rule, a metadata flag (frontmatter, a persona file's own gate), a runtime check, or a script that enforces the rule without depending on a future agent noticing a sentence. When more than one mechanism would work, prefer the strongest the situation allows -- a check that fails CI over a lint over a canonical helper over a runtime assertion -- since agents copy whatever the surrounding code already does, and a weaker guard becomes the next template. Propose the structural fix in place of, or alongside, a skill-file edit when one is genuinely available; fall back to prose only when the fix truly requires judgment a mechanism can't encode.
3. Draft the smallest concrete edit that would have caught or prevented this -- a bullet added to an existing section, a row in an existing table, a tightened sentence -- matching that file's existing structure, tone, and density. Do not invent a new section unless nothing existing is even remotely related. This is a draft, not yet applied.
4. Present the proposal for confirmation before touching the file: the target skill path, the exact text to add or change, and a one-sentence reason. Use `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded); on Codex or another runtime without a blocking-question tool, present the proposal in chat and ask in plain text, then wait for the reply.
5. **On explicit approval:** apply the edit with `Edit`, preserving the rest of the file's structure and frontmatter untouched. Report what changed and where.
6. **On decline, no response, or an unattended/non-interactive run with no one to ask:** do not touch the skill file. Record the proposal in the Phase 5 report as declined or pending -- never silently drop it, and never apply it by treating silence as approval.
